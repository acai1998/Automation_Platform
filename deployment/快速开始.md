...
# 构建镜像
podman build -t automation-platform:latest -f deployment/Dockerfile .

# 使用高级部署脚本（需要 docker-compose.yml）
./deploy.sh production rolling automation-platform:latest

# 或者手动 Docker 运行（如果有 docker-compose.yml）
docker-compose up -d  # 需要先创建 docker-compose.yml
```
...
### docker-compose.yml 文件（高级配置）

**注意：** 以下内容仅适用于自定义 Docker 部署。推荐使用 `deployment/scripts/setup.sh` 进行快速部署。

如果您需要创建自定义 docker-compose.yml 文件，配置如何读取 `.env` 文件：
