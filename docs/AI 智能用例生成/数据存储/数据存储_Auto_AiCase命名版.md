...
将 AI 用例生成工作台相关数据统一归入 `Auto_AiCase...` 域。
...
| `test_case` | `Auto_AiCaseGeneratedCases` | AI 生成结果先作为草稿，并持续保存在 AI 工作台域内 |
...
用于保存 AI 生成后的用例草稿。该表是 AI 工作台自己的结构化用例表，不与自动化脚本资产表 `Auto_TestCase` 直接关联。
...
| 工作台结果资产层 | `Auto_AiCaseGeneratedCases`、`Auto_AiCaseGeneratedCaseSteps` | AI 工作台内部确认后的结构化用例资产 |
...
用户确认 / 编辑 / 废弃
  ↓
确认后的用例继续保存在 Auto_AiCaseGeneratedCases
```
...
- AI 生成用例确认后在 AI 工作台域内沉淀
- 后续如需和自动化脚本、测试任务、执行结果打通，应单独设计映射关系
...
### 2. 为什么 AI 生成用例不直接复用 `Auto_TestCase`

`Auto_TestCase` 是自动化测试脚本资产表，不是 AI 工作台生成用例的目标表。

同时，AI 生成结果存在临时性和不确定性，用户通常还需要筛选、修改、补充、废弃。如果直接复用 `Auto_TestCase`，会把“测试设计数据”和“自动化脚本资产”混在一起。
...
- `Auto_AiCaseGeneratedCases` 和 `Auto_AiCaseGeneratedCaseSteps` 承载生成用例草稿与确认后的工作台内结果
- `Auto_AiCaseRequirementPoints`、`Auto_AiCaseCoverageMappings`、`Auto_AiCaseRiskItems` 承载覆盖率和风险分析
- `Auto_TestCase` 属于自动化脚本资产表，不作为 AI 生成用例的复用目标表
